...
public class PanelBill extends JPanel{
	...
	private static JButton btnDiscount, btnDel, btnPay, btnSave, btnMove;
	...
	public void new_PanePay() {
			...
			public void actionPerformed(ActionEvent e) {
				if(billBo.getBill() != null) {
					try {
						...
						billBo.updateBill((Float.parseFloat(discount)/100), false);
						setlblPay();
					} catch (Exception e2) {
					}
				}
		...
		panePay.add(btnSave);
		btnSave.setPreferredSize(new Dimension(155,30));
		...
		btnMove = new JButton("Chuyển bàn", new ImageIcon(PanelBill.class.getResource("/imgs/send-24.png")));
		btnMove.setFont(fontbtn);
		btnMove.setPreferredSize(new Dimension(155,30));
		btnMove.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				CafeFrame.setPanelTableMove(billBo.getTableId());
			}
		});
		panePay.add(btnMove);
		layout.putConstraint(SpringLayout.WEST, btnMove, 5, SpringLayout.HORIZONTAL_CENTER, panePay);
		layout.putConstraint(SpringLayout.SOUTH, btnMove, -50, SpringLayout.SOUTH, panePay);
	...
	public static void setPaneHead(TableFood tbFood) {
		...
		if(tbFood.getStatus()) {
			lblStatus.setText("<html><b>Trạng thái:</b> Đang phục vụ !</html>");
			lblTime.setText("<html><b>Giờ đến:</b> "+TimeFormat.format(bill.getTimeIn())+"</html>");
			refeshModelTable();
		}
		else{
			lblStatus.setText("<html><b>Trạng thái:</b> Trống !</html>");
			modelTb.setRowCount(0);
			lblTime.setText("<html><b>Giờ đến:</b></html>");
			setVisibleLbl(false);
		}
		refreshBtn();
	}
	...
	public static void addFood(Food food) throws SQLException {
		if(billBo.getBill() == null) {
			if(Main.checkConfirm("Add Bill", "Tạo hóa đơn mới?")) {
				setVisibleLblPay(false);
				billBo.addBill();
				PanelTable.setStatusTable(billBo.getTableId(), true);
			} else return;
		}
	...
	public static void saveBill(boolean b) {
		...
		billBo.updateBill(billBo.getBill().getDiscount(), b);
		refreshBtn();
	}
	...
	public static boolean checkIsSaveBill() {
		return billBo.getBillInforBo().isEdit();
	}
	...
	public static void refreshBtn() {
		if(billBo.getBill() != null) {
			if(billBo.getBillInforBo().isEmpty()) {
				...
				btnSave.setEnabled(checkIsSaveBill());
	...
	public static void finishPay(int k) {
		...
		if(k < tt) {
			...
			lblStatus.setText("<html><b>Trạng thái:</b> Đã thanh toán !</html>");
			PanelTable.setStatusTable(billBo.getTableId(), false);
			saveBill(true);
			refreshBtn();
		}
	}
