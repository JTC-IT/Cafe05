...
public class BillBo {
	...
	public int addBill() throws SQLException {
		int id = billDao.InsertBill(tableId, 0);
		if(id > 0) {
			ListBill.add(new Bill(id, new Timestamp(new Date().getTime()), new Timestamp(new Date().getTime()), tableId, 0, false));
			hashMap.put(id, new BillInforBo(id));
		}
		return id;
	}
	
	public boolean updateBill(float discount, boolean status){
		Bill bill = getBill();
		if(billDao.UpdateBill(bill.getId(), tableId, discount, status)) {
			if(status) {
	...
	public void moveTable(int tableNew) {
		Bill bill = getBill();
		billDao.UpdateBill(bill.getId(), tableNew, bill.getDiscount(), false);
